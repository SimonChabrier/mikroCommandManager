...
const loadCommands = async () => {
    try {
        const response = await fetch('fake_commands.json');
        ...
        commandsList.innerHTML = '<div class="bg-red-900/30 border border-red-800 text-red-300 px-4 py-3 rounded-lg text-sm">Erreur lors du chargement des commandes.</div>';
    }
};
...
const renderCommands = () => {
    ...
    if (filteredCommands.length === 0) {
        commandsList.innerHTML = '<div class="bg-gray-900/50 border border-gray-800 text-gray-400 px-4 py-3 rounded-lg text-sm text-center">Aucune commande trouvée.</div>';
